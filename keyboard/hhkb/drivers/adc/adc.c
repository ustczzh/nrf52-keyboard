...
void adc_init() {
    ...
    // 注册buffer
    err_code = nrf_drv_saadc_buffer_convert(adc_buffer[0], SAMPLES_BUFFER_LEN);
    APP_ERROR_CHECK(err_code);

    err_code = nrf_drv_saadc_buffer_convert(adc_buffer[1], SAMPLES_BUFFER_LEN);
    APP_ERROR_CHECK(err_code);
}

void adc_start() { nrf_drv_saadc_sample(); }
